...
# fission package create --spec --name afl-result-sentiment-bluesky-pkg \
# --source ./afl_result_sentiment_bluesky/__init__.py \
# --source ./afl_result_sentiment_bluesky/afl_result_sentiment_bluesky.py \
# --source ./afl_result_sentiment_bluesky/requirements.txt \
# --source ./afl_result_sentiment_bluesky/build.sh \
# --env python39x --buildcmd './build.sh'

# fission fn create --spec --name afl-result-sentiment-bluesky \
# --pkg afl-result-sentiment-bluesky-pkg \
# --env python39x \
# --entrypoint "afl_result_sentiment_bluesky.main" \
# --specializationtimeout 180 \
# --secret elastic-secret 

# fission route create --spec --name afl-result-sentiment-bluesky \
# --function afl-result-sentiment-bluesky \
# --method GET \
# --url /afl/result/sentiment/bluesky \
# --createingress

# kubectl port-forward service/router -n fission 8080:80

# fission fn log -f --name afl-result-sentiment-bluesky
# curl -k http://localhost:8080/afl/result/sentiment/bluesky | jq
...
def main() -> Dict[str, Any]:
    ...
    """Calculate AFL team sentiment scores (average and total) grouped by week
        and Retrieve match records for these teams from bluesky from Elasticsearch
    ...
    sentiment_result: Dict[str, Any] = es_client.search(index='afl_bluesky_sentiment-18*', body=sentiment_query)
    ...
    current_app.logger.info('Executing match result from bluesky analysis query')
